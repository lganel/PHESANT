...
only_males_file <- "/gscmnt/gc2719/halllab/users/lganel/src/PHESANT/should_only_be_in_males.tsv"
only_females_file <- "/gscmnt/gc2719/halllab/users/lganel/src/PHESANT/should_only_be_in_females.tsv"

# root_file <- "neale_lab_parsed_and_restricted_to_QCed_samples"
root_file <- "/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/ukb41102.nealesamps.nowithdraw.phesant_head.noempty"
...
pheno_summary <- read.table(pheno_summary_file, sep='\t', quote="", header=TRUE, stringsAsFactors=FALSE, comment.char="")
for(i in 2:4) {
	pheno_summary_file <- paste0(root_file, "_cat_variables_both_sexes_phesant_recodings_remove_sex_specific.", i, "_phenosummary.tsv")
	pheno_summary <- rbind(pheno_summary, read.table(pheno_summary_file, sep='\t', quote="", header=TRUE, stringsAsFactors=FALSE, comment.char=""))
}

pheno_summary_file <- paste0(root_file, "_cts_both_sexes_phenosummary.tsv")
pheno_summary_cts <- read.table(pheno_summary_file, sep='\t', quote="", header=TRUE, stringsAsFactors=FALSE, comment.char="")
pheno_summary <- rbind(pheno_summary, read.table(pheno_summary_file, sep='\t', quote="", header=TRUE, stringsAsFactors=FALSE, comment.char=""))
...
fwrite(pheno_summary, file='/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/040220/phesant_output_combined_both_sexes_no_sex_specific_summary.tsv', quote=FALSE, sep='\t', row.names=TRUE)
fwrite(pheno, file='/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/040220/phesant_output_combined_both_sexes_no_sex_specific.tsv', quote=FALSE, sep='\t')
...
fwrite(pheno_summary, file='/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/040220/phesant_output_combined_males_summary.tsv', quote=FALSE, sep='\t', row.names=TRUE)
fwrite(pheno, file='/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/040220/phesant_output_combined_males.tsv', quote=FALSE, sep='\t')
...
fwrite(pheno_summary, file='/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/040220/phesant_output_combined_females_summary.tsv', quote=FALSE, sep='\t', row.names=TRUE)
fwrite(pheno, file='/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/040220/phesant_output_combined_females.tsv', quote=FALSE, sep='\t')
