...
root_file <- "/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/output."
...
get_raw_cts <- function(cts_variable, file_header, file="/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease.tsv", single=TRUE)
{	
...
change_values <- function(codings, data_frame, coding_info_file) { # LG: Edited this function to allow for the length(codings)==0 and length(codings)==1 cases. However, it's untested for length(codings)==0
	...
	if(length(codings)==1) {
	  reassignment_matrices=list(t(reassignment_matrices))
	}
	...
	for(i in 1:length(reassignment_matrices)) {
		#print(paste0('Reassignment matrix',i,': ',codings[[i]])) ## DEBUG
...
prep_for_cbind=function(cat_df,to_keep) { # Handle the special cases where we want to write out categorical variable data frames with zero or one column
  if(sum(to_keep)==0) {
    return(data.frame(row.names=1:nrow(cat_df)))
  }
  cat_df_out=cat_df[,which(to_keep)]
  if(sum(to_keep)==1) {
    cat_df_out=data.frame(cat_df_out)
    colnames(cat_df_out)=names(which(to_keep))
  }
  return(cat_df_out)
}
...
file <- "/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease.tsv"
...
outfile_single <- "/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease_cts_single.tsv"
#system(paste0("awk -F $'\t' -v OFS=$'\t' '{print ", paste0("$", single_cts_columns, collapse=","), "}' ", file, " > ", outfile_single))
outfile_multi <- "/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease_cts_multi.tsv"
...
cts_multi <- data.frame(userId=cts_multi$eid, sapply(cts_multi_to_average_over, average_over_cts_multi, cts_multi))
...
colnames(cts_columns)[1]='userId'
cts_columns <- merge(cts_multi, cts_columns, by='userId')

# Pull out the make and female userIds for out application.
males <- system(paste0("/usr/bin/awk 'BEGIN { FS=\"\t\" } $3==1{ print $1 }' ", root_file, 1, ".tsv"), intern=TRUE)
females <- system(paste0("/usr/bin/awk 'BEGIN { FS=\"\t\" } $3==0{ print $1 }' ", root_file, 1, ".tsv"), intern=TRUE)
...
fwrite(cts_output[,to_keep_all_sexes_cts], file="/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease_cts_irnt.tsv", sep='\t')
fwrite(cts_output_males[, to_keep_males_cts], file="/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease_cts_irnt_males.tsv" , sep='\t')
fwrite(cts_output_females[, to_keep_females_cts], file="/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease_cts_irnt_females.tsv" , sep='\t')
...
fwrite(cts_output_raw[,to_keep_all_sexes_cts_raw], file="/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease_cts_raw.tsv", sep='\t')
fwrite(cts_output_males_raw[, to_keep_males_cts_raw], file="/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease_cts_raw_males.tsv" , sep='\t')
fwrite(cts_output_females_raw[, to_keep_females_cts_raw], file="/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease_cts_raw_females.tsv" , sep='\t')
...
for(i in 1:4)
{
	...
	fwrite(cbind(all_sexes$userId, prep_for_cbind(cat_ordered_all_sexes,to_keep_all_sexes_ordered), prep_for_cbind(cat_unordered_all_sexes,to_keep_all_sexes)), 
		sep='\t', file=paste0('/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease_cat_variables_both_sexes.', i, '.tsv'))
	fwrite(cbind(PHESANT_males$userId, prep_for_cbind(cat_ordered_males,to_keep_males_ordered), prep_for_cbind(cat_unordered_males,to_keep_male)), 
		sep='\t', file=paste0('/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease_cat_variables_males.', i, '.tsv'))
	fwrite(cbind(PHESANT_females$userId, prep_for_cbind(cat_ordered_females,to_keep_females_ordered), prep_for_cbind(cat_unordered_females,to_keep_female)), 
		sep='\t', file=paste0('/gscmnt/gc2802/halllab/lganel/ukbb/mt_cn/phesant/ukb34322.nealesamps.nowithdraw.phesant_head.nodisease_cat_variables_females.', i, '.tsv'))
